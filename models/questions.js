...
class Questions {
  ...
  async setAnswerRight(questionId, answerId, user ){
    ...
    if(!user.email === user.owner.email){
      return false //Para verfiicar que sea quien creo la pregunta
    }

    for( let key in answers ){
      answers[key].correct = (key === answerId) //Recorriendo la respuesta correcta que estamos iterando
    }
